    ...
    def get_compute_metrics_fn(self):
        ...
        def compute_metrics(eval_preds):
            ...
            return {
                ...
                "accuracy": self.acc.compute(references=binary_labels, predictions=predictions)["accuracy"]
            }
...
class GenerativeTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            if super().unwrap_model(model)._get_name() in super().MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
